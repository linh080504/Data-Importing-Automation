...
from scrapling.engines.static import (
    _ASyncSessionLogic as AsyncFetcherSession,
    AsyncFetcherClient,
)
...
class TestFetcherSession:
    ...
    async def test_session_level_proxy_is_applied(self):
        ...
        async with AsyncFetcherSession(proxy=proxy) as session:
            with (
                patch.object(
                    session._async_curl_session, "request", new=AsyncMock()
                ) as mocked_request,
                patch(
                    "scrapling.engines.static.ResponseFactory.from_http_request",
    ...
    async def test_proxy_rotates_per_retry_attempt(self):
        ...
        async with AsyncFetcherSession(
            proxy_rotator=rotator, retries=2, retry_delay=0
        ) as session:
            with (
                patch.object(
                    session._async_curl_session, "request", new=AsyncMock()
            ...
            proxies_used = [
                call.kwargs["proxy"] for call in mocked_request.call_args_list
            ]
