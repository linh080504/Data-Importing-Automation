...
class TestSaveRetrieveRoundTrip:
    ...
    def _make_element(
        self, html_str="<div><p id='target' class='main'>Hello</p></div>"
    ):
    ...
    def test_url_isolation(self):
        ...
        try:
            storage_a = SQLiteStorageSystem(
                storage_file=db_path, url="https://site-a.com"
            )
            element = self._make_element()
            ...
            storage_b = SQLiteStorageSystem(
                storage_file=db_path, url="https://site-b.com"
            )
