...
def test_full_path_selector_no_duplicate_ids():
    ...
    assert (
        css_full.count("#target") == 1
    ), f"Duplicate #target in CSS full path: {css_full}"
    ...
    assert (
        xpath_full.count("@id='target'") == 1
    ), f"Duplicate @id='target' in XPath full path: {xpath_full}"
    assert (
        xpath_full.count("@id='main'") == 1
    ), f"Duplicate @id='main' in XPath full path: {xpath_full}"
    ...
    assert (
        len(result) == 1
    ), f"XPath '{xpath_full}' selected {len(result)} elements, expected 1"
    assert result.first.text == "Hello"
