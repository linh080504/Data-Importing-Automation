...
class TestMCPServer:
    ...
    async def test_bulk_get_tool(self, server, test_url):
        ...
        results = await server.bulk_get(
            urls=(test_url, test_url), extraction_type="html"
        )
    ...
    async def test_bulk_stealthy_fetch_tool(self, server, test_url):
        ...
        result = await server.bulk_stealthy_fetch(
            urls=(test_url, test_url), headless=True
        )
        assert all(isinstance(r, ResponseModel) for r in result)
...
class TestSessionManagement:
    ...
    async def test_bulk_fetch_with_session(self, server, test_url):
        ...
        result = await server.open_session(
            session_type="dynamic", headless=True, max_pages=5
        )
        session_id = result.session_id

        responses = await server.bulk_fetch(
            urls=[test_url, test_url], session_id=session_id
        )
    ...
    async def test_open_session_with_custom_id(self, server):
        ...
        result = await server.open_session(
            session_type="dynamic", session_id="my-session", headless=True
        )
        assert isinstance(result, SessionCreatedModel)
    ...
    async def test_open_session_duplicate_id_raises(self, server):
        ...
        await server.open_session(
            session_type="dynamic", session_id="dupe", headless=True
        )
...
class TestScreenshot:
    ...
    async def test_screenshot_full_page_taller_than_viewport(self, server):
        ...
        with _serve_html(tall_html) as tall_url:
            opened = await server.open_session(session_type="dynamic", headless=True)
            try:
                viewport_result = await server.screenshot(
                    url=tall_url, session_id=opened.session_id, full_page=False
                )
                full_result = await server.screenshot(
