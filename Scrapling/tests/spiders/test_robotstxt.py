...
def make_fetch_fn(status: int = 200, content: str = "", encoding: str = "utf-8"):
    ...
    async def _fetch(url: str, sid: str) -> MockResponse:
        ...
        return MockResponse(
            status=status, body=content.encode(encoding), encoding=encoding
        )
...
class TestCanFetch:
    ...
    async def test_allow_directive_overrides_disallow(self):
        ...
        assert (
            await mgr.can_fetch("https://example.com/secret/public.html", "s1") is True
        )
        assert (
            await mgr.can_fetch("https://example.com/secret/private.html", "s1")
            is False
        )
...
class TestGetDelayDirectives:
    ...
    async def test_url_path_does_not_affect_result(self):
        ...
        r2 = await mgr.get_delay_directives(
            "https://example.com/deep/path/page.html", "s1"
        )
...
class TestCacheAndConcurrency:
    ...
    async def test_cached_domain_not_refetched(self):
        ...
        async def counting_fetch(url: str, sid: str) -> MockResponse:
            ...
            fetch_count += 1
            return MockResponse(
                status=200, body=ROBOTS_BASIC.encode(), encoding="utf-8"
            )
    ...
    async def test_concurrent_calls_consistent_results(self):
        ...
        results = await asyncio.gather(
            *[mgr.can_fetch("https://example.com/admin/", "s1") for _ in range(6)]
        )
