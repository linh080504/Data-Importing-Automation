...
class MockSession:
    ...
    async def fetch(self, url: str, **kwargs):
        ...
        resp = MockResponse(
            status=self._response.status, body=self._response.body, url=url
        )
        return resp
...
class TestIsDomainAllowed:
    ...
    def test_subdomain_match(self):
        ...
        assert (
            engine._is_domain_allowed(Request("https://sub.example.com/page")) is True
        )
        assert (
            engine._is_domain_allowed(Request("https://deep.sub.example.com/x")) is True
        )
...
class TestProcessRequest:
    ...
    async def test_retry_clears_proxy_kwargs(self):
        ...
        request = Request(
            "https://example.com", sid="default", proxy="http://proxy:8080"
        )
        await engine._process_request(request)
...
class TestPrefetchRobotsTxt:
    ...
    async def test_prefetch_uses_start_urls(self):
        ...
        spider = MockSpider(
            robots_txt_obey=True, start_urls=["https://example.com/page1"]
        )
        engine = _make_engine(spider=spider)
