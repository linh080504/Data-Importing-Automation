...
def __Request_and_Save(
    ...
    **kwargs,
    ...
    Convertor.write_content_to_file(
        response, str(output_path), css_selector, main_content_only=ai_targeted
    )
...
def __BuildRequest(
    ...
    if "impersonate" in kwargs and "," in (kwargs.get("impersonate") or ""):
        kwargs["impersonate"] = [
            browser.strip() for browser in kwargs["impersonate"].split(",")
        ]
...
def _common_http_options(f):
    ...
    decorators = [
        ...
        option(
            "--proxy", help='Proxy URL in format "http://username:password@host:port"'
        ),
        option(
            "--timeout",
            type=int,
        ...
        option(
            "--cookies", help='Cookies string in format "name1=value1; name2=value2"'
        ),
        option(
            "--headers",
            "-H",
...
def _common_browser_options(f):
    ...
    decorators = [
        ...
        option(
            "--locale",
            default=None,
            help="Specify user locale. Defaults to the system default locale.",
        ),
...
def _data_options(f):
    ...
    decorators = [
        option(
            "--json", "-j", help="JSON data to include in the request body (as string)"
        ),
        option(
            "--data",
            "-d",
...
def get(
    ...
    __http_command(
        "get", url, output_file, css_selector, ai_targeted=ai_targeted, **kwargs
    )
...
def post(
    ...
    __http_command(
        "post", url, output_file, css_selector, ai_targeted=ai_targeted, **kwargs
    )
...
def put(
    ...
    __http_command(
        "put", url, output_file, css_selector, ai_targeted=ai_targeted, **kwargs
    )
...
def delete(
    ...
    __http_command(
        "delete", url, output_file, css_selector, ai_targeted=ai_targeted, **kwargs
    )
