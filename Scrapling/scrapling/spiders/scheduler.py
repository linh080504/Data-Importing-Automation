...
class Scheduler:
    ...
    async def enqueue(self, request: Request) -> bool:
        ...
        fingerprint = request.update_fingerprint(
            self._include_kwargs, self._include_headers, self._keep_fragments
        )
    ...
    def snapshot(self) -> Tuple[List[Request], Set[bytes]]:
        ...
        sorted_items = sorted(
            self._pending.values(), key=lambda x: (x[0], x[1])
    ...
    def restore(self, data: "CheckpointData") -> None:
        ...
        log.info(
            f"Scheduler restored: {len(data.requests)} requests, {len(data.seen)} seen"
        )
