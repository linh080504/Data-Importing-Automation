...
class Spider(ABC):
    ...
    def __init__(
        ...
        formatter = logging.Formatter(
            fmt=self.logging_format.format(spider_name=self.name),
            datefmt=self.logging_date_format,
        )
        ...
        try:
            ...
            raise SessionConfigurationError(
                f"Error in {self.__class__.__name__}.configure_sessions(): {e}"
            ) from e

        if len(self._session_manager) == 0:
            raise SessionConfigurationError(
                f"{self.__class__.__name__}.configure_sessions() did not add any sessions"
            )
    ...
    async def __run(self) -> CrawlResult:
        ...
        try:
            self._engine = CrawlerEngine(
                self, self._session_manager, self.crawldir, self._interval
            )
    ...
    def start(self, use_uvloop: bool = False, **backend_options: Any) -> CrawlResult:
        ...
        try:
            return anyio.run(
                self.__run, backend="asyncio", backend_options=backend_options
            )
        finally:
