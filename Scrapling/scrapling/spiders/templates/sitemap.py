...
class SitemapSpider(Spider):
    ...
    def _decompress(body: bytes, content_type: Optional[str]) -> bytes:
        if (content_type and ("gzip" in content_type.lower())) or (
            body[:2] == _GZIP_MAGIC
        ):
            out = bytearray()
    ...
    async def _parse_sitemap(
        ...
        for child_url in result.sitemaps:
            if self.sitemap_follow is not None and not self.sitemap_follow.matches(
                child_url
            ):
                continue
    ...
    @staticmethod
    def _dispatch(
        response: "Response", url: str, rules: List[CrawlRule]
    ) -> Optional[Request]:
        if not rules:
