...
def _compile_patterns(
    patterns: Union[str, Pattern[str], PatternInput, None],
) -> Tuple[Pattern[str], ...]:
    if not patterns:
        return ()
...
class LinkExtractor:
    ...
    def __init__(
        ...
        self.allow_domains: Tuple[str, ...] = tuple(
            d.lower() for d in _to_str_tuple(allow_domains)
        )
        self.deny_domains: Tuple[str, ...] = tuple(
            d.lower() for d in _to_str_tuple(deny_domains)
        )
        ...
        self.deny_extensions: Set[str] = set(
            (ext.lower().lstrip(".") for ext in deny_extensions)
            if deny_extensions is not None
            else IGNORED_EXTENSIONS
        )
    ...
    def extract(self, response: "Response") -> List[str]:
        ...
        out: List[str] = []
        search_selector = "| ".join(
            [f".//{tag}/@{attr}" for tag in self.tags for attr in self.attrs]
        )
        for scope in scopes:
    ...
    def _url_passes(self, url: str) -> bool:
        ...
        if self.allow_domains or self.deny_domains:
            ...
            if self.allow_domains and not any(
                ...
                return False
            if self.deny_domains and any(
                host == d or host.endswith("." + d) for d in self.deny_domains
            ):
                return False
        return True
