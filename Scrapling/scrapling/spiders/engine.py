...
class CrawlerEngine:
    ...
    def __init__(
        ...
        if self.spider.development_mode:
            cache_dir = (
                self.spider.development_cache_dir
                or f".scrapling_cache/{self.spider.name}"
            )
            self._cache_manager: Optional[ResponseCacheManager] = ResponseCacheManager(
                cache_dir
            )
            log.warning(
                "Development mode enabled -- responses will be cached to disk and replayed on subsequent runs"
            )
        else:
    ...
    async def _get_domain_delay(self, request: Request) -> float:
        ...
        c_delay, r_rate = await robots_manager.get_delay_directives(
            request.url, request.sid
        )
    ...
    def _rate_limiter(self, domain: str) -> CapacityLimiter:
        ...
        if self.spider.concurrent_requests_per_domain:
            self._domain_limiters.setdefault(
                domain, CapacityLimiter(self.spider.concurrent_requests_per_domain)
            )
    ...
    async def _process_request(self, request: Request) -> None:
        ...
        if self._cache_manager and request._fp is not None:
            ...
            await self._cache_manager.put(
                request._fp, response, request._session_kwargs.get("method", "GET")
            )
        ...
        if await self.spider.is_blocked(response):
            ...
            if request._retry_count < self.spider.max_blocked_retries:
                ...
                new_request = await self.spider.retry_blocked_request(
    ...
    def _is_checkpoint_time(self) -> bool:
        ...
        ) >= self._checkpoint_manager.interval
    ...
    async def _prefetch_robots_txt(self) -> None:
        ...
        await self._robots_manager.prefetch(
            seed_urls, self.session_manager.default_session_id
        )
    ...
    async def crawl(self) -> CrawlStats:
        ...
        async with self.session_manager:
            ...
            self.stats.concurrent_requests_per_domain = (
            ...
            try:
                ...
                async with create_task_group() as tg:
                    while self._running:
                        ...
                        if (
                            self._checkpoint_system_enabled
                            and self._is_checkpoint_time()
                        ):
                            await self._save_checkpoint()
