...
class CrawlStats:
    ...
    def increment_status(self, status: int) -> None:
        self.response_status_count[f"status_{status}"] = (
            self.response_status_count.get(f"status_{status}", 0) + 1
        )

    def increment_response_bytes(self, domain: str, count: int) -> None:
        self.response_bytes += count
        self.domains_response_bytes[domain] = (
            self.domains_response_bytes.get(domain, 0) + count
        )
