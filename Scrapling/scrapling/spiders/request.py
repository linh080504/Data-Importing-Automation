...
class Request:
    def __init__(
        ...
        callback: (
            Callable[
                [Response], AsyncGenerator[Union[Dict[str, Any], "Request", None], None]
            ]
            | None
        ) = None,
        priority: int = 0,
    ...
    def update_fingerprint(
        ...
        if include_headers:
            headers = (
                self._session_kwargs.get("headers")
                or self._session_kwargs.get("extra_headers")
                or {}
            )
            processed_headers = {}
            # Some header normalization
            for key, value in headers.items():
                processed_headers[_convert_to_bytes(key.lower()).hex()] = (
                    _convert_to_bytes(value).hex()
                )
            data["headers"] = tuple(processed_headers.items())

        fp = hashlib.sha1(
            orjson.dumps(data, option=orjson.OPT_SORT_KEYS), usedforsecurity=False
        ).digest()
        self._fp = fp
        return fp
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<Request({self.url}) priority={self.priority} callback={callback_name}>"
        )
