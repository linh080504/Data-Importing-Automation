...
class Selector(SelectorsGeneration):
    ...
    def __init__(
        ...
        **_,
        ...
        if root is None:
            ...
            self._root = cast(
                HtmlElement,
                fromstring(body or "<html/>", parser=parser, base_url=url or ""),
            )
    ...
    def __elements_convertor(
        self, elements: List[HtmlElement | _ElementUnicodeResult]
    ) -> "Selectors":
    ...
    def get_all_text(
        ...
        for text_node in cast(
            list[_ElementUnicodeResult], _find_all_text_nodes(self._root)
        ):
            text = str(text_node)
    ...
    def html_content(self) -> TextHandler:
        ...
        if self._is_text_node(self._root):
            return TextHandler(str(self._root))
        content = tostring(
            self._root, encoding=self.encoding, method="html", with_tail=False
        )
    ...
    def next(self) -> Optional["Selector"]:
        ...
        return (
            self.__element_convertor(next_element) if next_element is not None else None
        )
    ...
    def previous(self) -> Optional["Selector"]:
        ...
        return (
            self.__element_convertor(prev_element) if prev_element is not None else None
        )
    ...
    def relocate(
        ...
        element: Union[Dict, HtmlElement, "Selector"],
        percentage: int,
        selector_type: Literal[False] = False,
    ) -> List[HtmlElement]: ...
            ...
            if score_table[highest_probability] and highest_probability >= percentage:
                if log.getEffectiveLevel() < 20:
                    ...
                    for percent in tuple(sorted(score_table.keys(), reverse=True))[:5]:
                        log.debug(
                            f"{percent} -> {self.__elements_convertor(score_table[percent])}"
                        )
        ...
        **kwargs: str,
        ...
        for arg in args:
            if isinstance(arg, str):
                ...
                arg = cast(
                    Iterable, arg
                )  # Type narrowing for type checkers like pyright
    ...
    def __calculate_similarity_score(
        ...
        if original["text"]:
            score += SequenceMatcher(
                None, original["text"], data.get("text") or ""
            ).ratio()
            checks += 1
        ...
        if original.get("siblings"):
            score += SequenceMatcher(
                None, original["siblings"], data.get("siblings") or []
            ).ratio()
            checks += 1
