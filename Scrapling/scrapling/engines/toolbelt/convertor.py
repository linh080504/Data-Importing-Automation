...
class ResponseFactory:
    ...
    def __extract_browser_encoding(
        cls, content_type: str | None, default: str = "utf-8"
    ) -> str:
                    ...
                    history.insert(
                        ...
                        Response(
                            **{
                                ...
                                "content": "",
                                "status": (
                                    current_response.status if current_response else 301
                                ),
                                "reason": (
                                    (
                                ...
                                "encoding": (
                                    cls.__extract_browser_encoding(
    ...
    def from_playwright_response(
        ...
        history = (
            cls._process_response_history(first_response, parser_arguments)
            if collect_history
            else []
        )
        try:
        ...
        response = Response(
            **{
                ...
                "encoding": encoding,
                "cookies": (
                    tuple(dict(cookie) for cookie in page.context.cookies())
                    if page
                    else {}
                ),
        ...
        if xhr_captured:
            response.captured_xhr = [
                cls.from_playwright_response(None, p, None, {}, collect_history=False)
                for p in xhr_captured
            ]
        return response
    ...
    def _get_page_content(cls, page: SyncPage, max_retries: int = 20) -> str:
        ...
        raise RuntimeError(
            f"Failed to retrieve the page content after retrying for {max_retries * 500}ms."
        )

    @classmethod
    async def _get_async_page_content(
        cls, page: AsyncPage, max_retries: int = 20
    ) -> str:
        """
        A workaround for the Playwright issue with `page.content()` on Windows. Ref.: https://github.com/microsoft/playwright/issues/16108
        :param page: The page to extract content from.
    ...
    async def from_async_playwright_response(
        ...
        history = (
            await cls._async_process_response_history(first_response, parser_arguments)
            if collect_history
            else []
        )
        try:
            if page and "html" in (await final_response.all_headers()).get(
                "content-type", ""
            ):
        ...
        response = Response(
            **{
                ...
                "encoding": encoding,
                "cookies": (
                    tuple(dict(cookie) for cookie in await page.context.cookies())
                    if page
                    else {}
                ),
        ...
        if xhr_captured:
            response.captured_xhr = [
                await cls.from_async_playwright_response(
                    None, p, None, {}, collect_history=False
                )
                for p in xhr_captured
            ]
        return response
    ...
    def from_http_request(
        response: CurlResponse, parser_arguments: Dict, meta: Optional[Dict] = None
    ) -> Response:
        ...
        return Response(
            **{
                ...
                "request_headers": (
                    dict(response.request.headers) if response.request else {}
                ),
