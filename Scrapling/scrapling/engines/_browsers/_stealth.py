...
from scrapling.engines._browsers._base import (
    SyncSession,
    AsyncSession,
    StealthySessionMixin,
)
from scrapling.engines._browsers._validators import (
    validate_fetch as _validate,
    StealthConfig,
)

__CF_PATTERN__ = re_compile(
...
class StealthySession(SyncSession, StealthySessionMixin):
            ...
            if challenge_type == "non-interactive":
                ...
                if challenge_type != "embedded":
                    ...
                    while (
                        ...
                        in ResponseFactory._get_page_content(page)
                    ):
                ...
                if not iframe or not outer_box:
                    if "<title>Just a moment...</title>" not in (
                        ResponseFactory._get_page_content(page)
                    ):
                        log.info("Cloudflare captcha is solved")
                        return None
                ...
                page.mouse.click(
                    captcha_x, captcha_y, delay=randint(100, 200), button="left"
                )
    ...
    def fetch(self, url: str, **kwargs: Unpack[StealthFetchParams]) -> Response:
        ...
        request_headers_keys = (
            {h.lower() for h in params.extra_headers.keys()}
                ...
                try:
                    ...
                    if params.wait_selector:
                        try:
                            ...
                            log.error(
                                f"Error waiting for selector {params.wait_selector}: {e}"
                            )
                ...
                try:
                    ...
                    if params.wait_selector:
                        try:
                            ...
                            await waiter.first.wait_for(
                                state=params.wait_selector_state
                            )
                            await self._wait_for_page_stability(
                                page, params.load_dom, params.network_idle
                            )
