...
class PlaywrightConfig(Struct, kw_only=True, frozen=False, weakref=True):
    ...
    def __post_init__(self):  # pragma: no cover
        ...
        if self.page_setup and not callable(self.page_setup):
            raise TypeError(
                f"page_setup must be callable, got {type(self.page_setup).__name__}"
            )
...
def validate_fetch(
    ...
    if overrides:
        ...
        validated_dict = {
            field: getattr(validated_config, field)
...
def validate(
    params: Dict, model: type[PlaywrightConfig] | type[StealthConfig]
