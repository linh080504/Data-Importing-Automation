...
def _select_random_browser(
    impersonate: ImpersonateType,
) -> Optional[BrowserTypeLiteral]:
    """
...
class _ConfigurationLogic(ABC):
    ...
    def _merge_request_args(self, **method_kwargs) -> Dict[str, Any]:
        ...
        impersonate = self._get_param(
            method_kwargs, "impersonate", self._default_impersonate
        )
        ...
        final_args = {
            ...
            "proxy_auth": self._get_param(
                method_kwargs, "proxy_auth", self._default_proxy_auth
            ),
            "timeout": self._get_param(method_kwargs, "timeout", self._default_timeout),
            "allow_redirects": self._get_param(
                method_kwargs, "follow_redirects", self._default_follow_redirects
            ),
            "max_redirects": self._get_param(
                method_kwargs, "max_redirects", self._default_max_redirects
            ),
    ...
    def _headers_job(
        self, url, headers: Dict, stealth: bool, impersonate_enabled: bool
    ) -> Dict:
        """
                ...
                final_headers.update(
                    {
                        ...
                        if k.lower() not in headers_keys
                    }
                )  # Don't overwrite user-supplied headers

        elif (
            "user-agent" not in headers_keys and not impersonate_enabled
        ):  # pragma: no cover
            final_headers["User-Agent"] = __default_useragent__
            log.debug(
                f"Can't find useragent in headers so '{final_headers['User-Agent']}' was used."
            )
...
class _SyncSessionLogic(_ConfigurationLogic):
    ...
    def _make_request(
        ...
        selector_config = (
            self._get_param(kwargs, "selector_config", self.selector_config)
            or self.selector_config
        )
        ...
        try:
            for attempt in range(max_retries):
                ...
                request_args = self._merge_request_args(
                    stealth=stealth, proxy=proxy, **kwargs
                )
                try:
                    response = session.request(method, **request_args)
                    assert response is not None
                    result = ResponseFactory.from_http_request(
                        response, selector_config, meta={"proxy": proxy}
                    )
                    return result
...
class _ASyncSessionLogic(_ConfigurationLogic):
    ...
    def post(
        self, url: str, **kwargs: Unpack[DataRequestParams]
    ) -> Awaitable[Response]:
        """
        Perform a POST request.
...
class FetcherSession:
    ...
    def __enter__(self) -> _SyncSessionLogic:
        ...
        if self._client is None:
            ...
            config = {
                k.replace("_default_", ""): getattr(self, k)
