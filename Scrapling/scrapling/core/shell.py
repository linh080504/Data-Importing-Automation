...
class CustomShell:
    ...
    def __init__(self, code, log_level="debug"):
        from IPython.terminal.embed import (
            InteractiveShellEmbed as __InteractiveShellEmbed,
        )
...
Type 'exit' or press Ctrl+D to exit.
    ...
    def get_namespace(self):
        ...
        stealthy_fetch = self.create_wrapper(
            self.__StealthyFetcher.fetch, signature_name="stealthy_fetch"
        )
        curl2fetcher = self.create_wrapper(
            self._curl_parser.convert2fetcher, get_signature=False
        )
...
class Convertor:
    ...
    def _extract_content(
        ...
        if not page or not isinstance(page, Selector):  # pragma: no cover
            ...
            pages = (
                [page] if not css_selector else cast(Selectors, page.css(css_selector))
            )
            for page in pages:
