...
def export_crawl_job(job_id: str, payload: ExportRequest, db: Session = Depends(get_db)) -> ExportResponse:
    ...
    if payload.format == "xlsx":
        export_clean_records_to_xlsx(clean_records, template_columns=template.columns)
    else:
        export_clean_records_to_csv(clean_records, template_columns=template.columns)
